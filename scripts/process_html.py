...
def parse_text_only(in_file, urlid):
    page = open(in_file, 'r')
    soup = bs(page)
...
def bs_from_file(filename):
    page = open(filename, 'r')
    return bs(page)

def tags_by_name(soup, name):
    return soup.findAll(name)

def char_count(tags):
    if not tags:
        return 0
    char_counts = map(lambda k: len(str(k)), tags)
    return reduce(lambda prev, curr: prev + curr, char_counts)

def parse_by_name(soup, tag_name):
    if tag_name == 'p':
        tags = parse_text(soup)
    else:
        tags = tags_by_name(soup, tag_name)
    return { 'count': len(tags), 'char_count': char_count(tags) }
